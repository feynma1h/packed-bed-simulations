...
global lobes d r L zMin innerRadius

% ALL USER DEFINED VARIABLES HERE
lobes = 4; % number of lobes
d = (2-0.93)/2; r = 0.93/2; L = 2; % N-lobe particle dimensions
zMin = 0; % defines the z-location of tube bottom
R = 4.7; % radius of tube
dH = 0.5; % distance b/w planes

% ---MAIN CODE BEGINS HERE---
innerRadius = d*cos(pi/lobes) + heaviside(asin(d/R*sin(pi/lobes)))*sqrt(r^2-d^2*(sin(pi/lobes))^2);
H = getBedHeight(data);
N = ceil(H/dH);
...
for i = 1:N
    ...
    for j = 1:size(data, 1)
        ...
        if CheckIntersect(center', theta, z, innerRadius) | any(CheckIntersect(lobeCenters, theta, z, r))
            X = []; Y = [];
            [X Y] = coordinates(center', theta, z, innerRadius);
            fill(X, Y, 'w', 'LineStyle', 'none')
            hold on
            for k = 1:size(lobeCenters, 2)
                X = []; Y = [];
                [X Y] = coordinates(lobeCenters(:, k), theta, z, r);
                fill(X, Y, 'w', 'LineStyle', 'none')
                hold on
            end
        end
...
function H = getBedHeight(data)
    ...
    for i = 1:size(data, 1)
        ...
        for j = 1:2
            for k = 1:2
                dummy(i) = max(dummy(i), center(3)+(-1)^j*L/2*n+(-1)^k*innerRadius*sqrt(l^2+m^2));
            end
        end
